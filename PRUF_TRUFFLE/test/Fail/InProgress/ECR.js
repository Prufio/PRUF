...
contract("ECR", (accounts) => {
  ...
  it("Should add users to Node 1000001-1000006 in AC_Manager", () => {
    ...
    return NODE_MGR.addUser("1000001", account4Hash, "1", { from: account1 })
      ...
      .then(() => {
        console.log("Account8 => 1000001");
        return NODE_MGR.addUser("1000001", account8Hash, "9", {
          from: account1,
        });
      })
  ...
  it("Should write asset2 in Node 1000001", async () => {
    return APP.newRecord(asset2, rgt2, "1000001", "100", { from: account2 });
  });

  it("Should write asset3 in Node 1000001", async () => {
    return APP.newRecord(asset3, rgt3, "1000001", "100", { from: account2 });
  });

  it("Should set asset3 into status 1", async () => {
    return APP.modifyStatus(asset3, rgt3, "1", { from: account2 });
  });

  it("Should set asset3 into status 3(stolen)", async () => {
    return APP.setLostOrStolen(asset3, rgt3, "3", { from: account2 });
  });

  it("Should write asset4 in Node 1000001", async () => {
  ...
  it("Should set asset4 into status 4(lost)", async () => {
    return APP.setLostOrStolen(asset4, rgt4, "4", { from: account2 });
  });

  it("Should write asset5 in Node 1000001", async () => {
    return APP.newRecord(asset5, rgt5, "1000001", "100", { from: account2 });
  });

  it("Should set asset5 into status 1", async () => {
    return APP.modifyStatus(asset5, rgt5, "1", { from: account2 });
  });

  it("Should transfer asset5 to 0x0", async () => {
    return APP.transferAsset(asset5, rgt5, rgt000, { from: account2 });
  });

  it("Should write asset6 in Node 1000003", async () => {
  ...
  it("Should fail becasue custodial contract does not hold token", async () => {
    ...
    console.log(
      "//**************************************BEGIN ECR FAIL BATCH (15)**********************************************/"
    );
    console.log(
      "//**************************************BEGIN setEscrow FAIL BATCH**********************************************/"
    );
    return ECR.setEscrow(asset6, account4Hash, "180", "6", { from: account4 });
  });

  it("Should make ECR unatuhorized", async () => {
    return STOR.enableContractForNode("ECR", "1000001", "0", { from: account1 })
    .then(() => {
      return STOR.enableContractForNode("ECR", "1000002", "0", {
        from: account1,
      });
    });
  });
  ...
  //3
  it("Should fail because user not authorized in Node", async () => {
    return ECR.setEscrow(asset2, account2Hash, "180", "6", { from: account3 });
  });

  it("Should auth account10 in Node 1000001 as robot", async () => {
    return NODE_MGR.addUser("1000001", account10Hash, "9", { from: account1 });
  });

  //4
  it("Should fail because usertype > 4 cannot set escrow < 49", async () => {
    return ECR.setEscrow(asset1, account2Hash, "180", "6", { from: account10 });
  });

  it("Should unauth account10 in Node 10 as robot", async () => {
  ...
  it("Should fail because you cannot set to recycled through escrow", async () => {
    return ECR.setEscrow(asset1, account2Hash, "180", "60", { from: account2 });
  });
  ...
  //         .then(() => {
  //             return STOR.enableContractForNode('ECR', '1000002', '0', { from: account1 })
  //         })
  // })
  ...
  // it('Should auth account10 in Node 10 as robot', async () => {
  //     return NODE_MGR.addUser('1000001', account10Hash, '9', { from: account1 })
  // })
  ...
  it("Should fail becasue custodial contract does not hold token", async () => {
    ...
    return ECR.endEscrow(asset6, { from: account4 });
  });
  ...
  it("Should make ECR unatuhorized", async () => {
    return STOR.enableContractForNode("ECR", "1000001", "0", {
      from: account1,
    }).then(() => {
      return STOR.enableContractForNode("ECR", "1000002", "0", {
        from: account1,
  ...
  //8
  it("Should fail because user not authorized for Node", async () => {
    return ECR.endEscrow(asset1, { from: account5 });
  });
  ...
  it("Should fail usertype is not < 5", async () => {
    return ECR.endEscrow(asset1, { from: account8 });
  });
  ...
  it("Should put asset1 into escrow", async () => {
    return ECR.setEscrow(asset1, account4Hash, "180", "6", { from: account2 });
  });
  ...
  it("Should take asset1 out of escrow", async () => {
    return ECR.endEscrow(asset1, { from: account4 });
  });
  it("Should set asset1 into status 1", async () => {
    return APP.modifyStatus(asset1, rgt1, "1", { from: account2 });
  });
