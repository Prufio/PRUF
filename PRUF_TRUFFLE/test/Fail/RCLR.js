...
contract("RCLR", (accounts) => {
  ...
  it("Should put asset2 into status 59(discardable)", async () => {
    return APP2_NC.modifyStatus(asset2, "59", { from: account4 });
  });
  ...
  it("Should write asset3 in Node 1000003", async () => {
    return APP_NC.newRecord(asset3, rgt3, "1000003", "100", { from: account4 });
  });

  it("Should pause RCLR", async () => {
    return RCLR.pause({ from: account1 });
  });

  //1
  it("Should fail becasue RCLR is paused", async () => {
    console.log(
      "//**************************************END RCLR SETUP**********************************************/"
    );
    console.log(
      "//**************************************BEGIN RCLR FAIL BATCH (4)**********************************************/"
    );
    console.log(
      "//**************************************BEGIN discard FAIL BATCH**********************************************/"
    );
    return MAL_APP.discard(asset1, { from: account4 });
  });

  it("Should unpause RCLR", async () => {
    return RCLR.unpause({ from: account1 });
  });
  ...
  it("Should fail because status must be 59(recyclable)", async () => {
    return A_TKN.discard(asset3, { from: account4 });
  });
  ...
  it("Should fail becasue RCLR is paused", async () => {
    ...
    console.log(
      "//**************************************BEGIN recycle FAIL BATCH**********************************************/"
    );
    return RCLR.recycle(asset1, rgt000, { from: account4 });
  });
  ...
  it("Should recycle asset2", async () => {
    return RCLR.recycle(asset2, rgt2, { from: account4 });
  });
