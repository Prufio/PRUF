...
contract('PRUF_FULL_TEST', accounts  => {
    ...
    it('Should deploy simpleEscrow', async () => {
        const PRUF_ECR_TEST = await PRUF_simpleEscrow.deployed();
    ...
    it('Should add and resolve contract addresses', async () => {

        return STOR.OO_addContract("PRUF_APP", APP.address, '3')
        .then(()=>{ 
        return STOR.OO_addContract("PRUF_NP", NP.address, '3')
        }).then(()=>{ 
        return STOR.OO_addContract("AC_MGR", MGR.address, '3')
        }).then(()=>{ 
        return STOR.OO_addContract("ECR", ECR.address, '3')
    })
