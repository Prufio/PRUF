...
contract FAUCET is ReentrancyGuard, Pausable, AccessControl {
    ...
    function BUY_PRUF() public payable nonReentrant whenNotPaused {
        ...
        uint256 amountToMint = msg.value * (_whiteList.tokensPerEth / 1 ether); //in wei
        ...
        require(
            amountToMint+(presaleCount) <= presaleLimit,
            "PP:PP: Purchase request exceeds total presale limit"
        );
        //^^^^^^^checks^^^^^^^^^

        presaleCount = amountToMint + presaleCount;

        whiteList[_msgSender()].maxEth = _whiteList.maxEth - msg.value; //reduce max purchasable by purchased amount
