...
//so these funtions would be accepting pre-hashed bytes32 instead of strings.
    ...
    function _NEW_RECORD (string memory _idx, string memory _reg, uint16 _assetClass, uint _countDownStart, string memory _desc) public payable {
        Storage.newRecord(keccak256(abi.encodePacked(msg.sender)), keccak256(abi.encodePacked(_idx)), keccak256(abi.encodePacked(_reg)), _assetClass, _countDownStart, keccak256(abi.encodePacked(_desc)));
    }
