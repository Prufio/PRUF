...
contract FrontEnd is PullPayment, Ownable, IERC721Receiver {
     ...
     * @dev Address Setters
     */
    function OO_setTokenAddress(address _assetContractAddress, address _assetClassContractAddress)
        external
        onlyOwner
    {
        require((_assetContractAddress != address(0)) && (_assetClassContractAddress != address(0)), "Invalid contract address");
        AssetClassTokenAddress = _assetClassContractAddress;
        AssetClassTokenContract = AssetClassTokenInterface(_assetClassContractAddress);
        AssetTokenAddress = _assetContractAddress;
        AssetTokenContract = AssetTokenInterface(_assetContractAddress);
    }
