...
//--------------------------------Internal Admin functions / onlyowner---------------------------------//
     ...
     */
    function ADMIN_setCosts (uint16 _class, uint _newRecordCost, uint _transferRecordCost, uint _createNoteCost, 
                                uint _cost4, uint _cost5, uint _forceModCost) onlyOwner external {
...
//-----------------------------------------------Private functions------------------------------------------------//
    ...
    function newRecorder(bytes32 _senderHash, bytes32 _recorder, bytes32 _lastRecorder) private view returns(bytes32, bytes32) {
        ...
        if ( ((registeredUsers[_recorder].userType == 1) || (_recorder == keccak256(abi.encodePacked(owner()))))
