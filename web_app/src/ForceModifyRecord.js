...
class ForceModifyRecord extends Component {
  ...
  constructor(props){
    ...
    this.getCosts = async () => {
      const self = this;
      if(self.state.costArray[5] > 0 || self.state.storage === ""){}else{for(var i = 0; i < 1; i++){
  ...
  render(){
    ...
    const _forceModifyRecord = () => {
      ...
      this.state.frontend.methods
        ...
        .send({ from: this.state.addr, value: this.state.costArray[5] }).on("error", function(_error){self.setState({error: _error});self.setState({result: _error.transactionHash});})
        .on("receipt", (receipt) => {
